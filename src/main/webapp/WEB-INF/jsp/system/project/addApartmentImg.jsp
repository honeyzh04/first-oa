...
<script>
    ...
    Setshowimg("#sj_img_box", apartmentId);
    ...
    $(".sjsubmit").click(function () {
        ...
        checkform("up_img1", "#box1", "#sj_img_box", apartmentId, 0)
    })

    // ajaxForm提交表单
    function checkform(objform, imgboxq, imgbox, apartmentId, imgarr) {
        var inpform = document.getElementById(objform);
        var inpimg = inpform.getElementsByClassName("imgtr");
        if (inpimg.length !== 0) {
            var options = {
                url: "../project/addApartmentImg.shtml",
                target: "#targetbox",
                        ...
                        success: function (res, error) {
                            ...
                            if (res !== null) {
                                ...
                                for (var i = 0; i < res.length; i++) {
                                    imghtml += '<tr> <td class="imgtd"><img src="../img/pro_apartment/' + apartmentId + '/' + res[i].img + '"></td> <td>' + res[i].img + '</td> <td onclick="Delimg(this,' + res[i].id + ')"><button class="btn btn-danger">删除</button></td>'
                                }
                                $(imgbox).html(imghtml)
                            }
                        }
