...
<body>
<form method="post" action="" id="up_img1" enctype="multipart/form-data" multipart="multipart">
    ...
    <div class="wrap">
        ...
        <div class="floatr">
            <button type="submit" class="btn btn-info sjsubmit">开始上传</button>
        </div>
...
<script>
    ...
    $(".sjsubmit").click(function () {
        ...
        checkform("up_img1", "#box1", "#sj_img_box", shopsId, 0)
    })

    // ajaxForm提交表单
    function checkform(objform, imgboxq, imgbox, shopsId, imgarr) {
        var inpform = document.getElementById(objform);
        var inpimg = inpform.getElementsByClassName("imgtr");
        if (inpimg.length !== 0) {
            var options = {
                url: "../project/addShopsImg.shtml",
                target: "#targetbox",
                        ...
                        success: function (res, error) {
                            ...
                            if (res !== null) {
                                ...
                                for (var i = 0; i < res.length; i++) {
                                    imghtml += '<tr> <td class="imgtd"><img src="../img/pro_shops/' + shopsId + '/' + res[i].img + '"></td> <td>' + res[i].img + '</td> <td onclick="Delimg(this,' + res[i].id + ')"><button class="btn btn-danger">删除</button></td>'
                                }
                                $(imgbox).html(imghtml)
                            }
                        }
    ...
    function Delimg(obj, id) {
        ...
        if (con == true) {
            ...
            $.ajax({
                url: "../project/deleteShopsImg.shtml?id=" + id,
                type: 'get',
                dataType: 'json',
                success: function (res, error) {
